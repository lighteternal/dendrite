...
import { isValidPMID, isValidDOI, isValidPMCID, normalizePMCID, formatCitation, buildFieldQuery, combineSearchTerms, chunkArray, extractErrorMessage, formatDateForAPI } from './api/utils.js';
...
async function handleSearchArticles(args) {
    const { query, max_results = 20, start = 0, sort, date_from, date_to } = args;
...
async function handleAdvancedSearch(args) {
    const { title, abstract, author, journal, mesh_terms, publication_types, boolean_operator = 'AND', max_results = 20 } = args;
    const queryParts = [];
    if (title)
        queryParts.push(buildFieldQuery(title, 'Title'));
    if (abstract)
        queryParts.push(buildFieldQuery(abstract, 'Abstract'));
    if (author)
        queryParts.push(buildFieldQuery(author, 'Author'));
    if (journal)
        queryParts.push(buildFieldQuery(journal, 'Journal'));
    if (mesh_terms && mesh_terms.length > 0) {
        const meshQuery = mesh_terms.map((term) => buildFieldQuery(term, 'MeSH Terms')).join(' OR ');
        queryParts.push(`(${meshQuery})`);
    }
    if (publication_types && publication_types.length > 0) {
        const ptQuery = publication_types.map((pt) => buildFieldQuery(pt, 'Publication Type')).join(' OR ');
